...
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB, RapidAPI services might allow larger files
...
const formSchema = z.object({
  imageFile: z
    ...
    .refine(
      ...
      "Only .jpg, .jpeg, .png, .webp, .gif formats are accepted."
    )
});
...
export default function RapidApiImageUploadForm({ onSubmit, isLoading }: RapidApiImageUploadFormProps) {
  ...
  const handleSubmit = async (values: ImageUploadFormValues) => {
    if (!previewUrl) {
      toast({
        variant: "destructive",
    ...
    <Card className="w-full shadow-xl bg-card/80 border border-border/50">
      <CardHeader>
        ...
        <CardDescription className="font-code text-muted-foreground/80">
          Select an image to perform a reverse search via RapidAPI.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription className="font-code text-xs text-muted-foreground/70">
                    Max 10MB. JPG, PNG, WEBP, GIF accepted.
                  </FormDescription>
            ...
            <div className="flex items-start text-xs text-muted-foreground/90 mt-1 p-3 bg-muted/30 rounded-md border border-border/20">
              ...
              <span>
                This tool uses a user-configured RapidAPI endpoint. Ensure you have a valid RAPIDAPI_KEY and RAPIDAPI_HOST set in your .env file.
                The success of the search depends on the specific RapidAPI service chosen and its capabilities.
              </span>
            </div>
